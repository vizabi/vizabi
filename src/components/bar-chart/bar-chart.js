define([
    ...
    var margin = { left: 55, right: 15, top: 15, bottom: 10 },
        ...
        render: function() {
            ...
            this.execute(function() {
                ...
                $.when(
                        ...
                        _.map(this.state.show, function(object, region) {
                            if (object.filter) {
                                return _.map(object.filter, function(d) {
                                    return { category: region, id: d }
                                });
                            } else {
                                return { category: region, id: undefined }
                            }
                        })
                    );

                    var things = _this.dataManager.getThing(show);
                    var ids = _.map(things, function(t) { return t.id; });
                    ...
                    chart.append('rect')
                        .attr('class', 'bar')
                        .attr('x', function(d) { return x(things[d].name); })
                        .attr('width', x.rangeBand())
                        .attr('y', function(d) { return y(data[d][time].v); })
                        .attr('height', function(d) { return height - y(data[d][time].v); });
                    ...
                    chart.append('text')
                        ...
                        .text(function(d) { return things[d].name; });

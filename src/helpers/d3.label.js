  ...
  return function d3_label() {
    ...
    var labelDragger = d3.behavior.drag()
      ...
      .on("drag", function(d, i) {
        ...
        var resolvedX = _xScale(cache.labelX0) + cache.labelX_ * _this.width;
        var resolvedY = _yScale(cache.labelY0) + cache.labelY_ * _this.height;
  ...
  //          });
    ...
    function updateLabel(d, index, valueX, valueY, scaledS, valueL, valueLST, duration, showhide, labelOffset, labelText) {
      ...
      _this.entityLabels.filter(function(f) {
        ...
        .each(function(groupData) {
          ...
          cached.labelX_ = _labelOffset[0] || (-cached.scaledS0 * .75 - 5) / _this.width;
          cached.labelY_ = _labelOffset[1] || (-cached.scaledS0 * .75 - 11) / _this.height;
          ...
          if(_labelOffset[0] == 0) {
            if(resolvedX - cached.contentBBox.width <= 0) { //check left
              ...
              resolvedX = resolvedX0 + cached.labelX_ * _this.width;
            } else if(resolvedX + 15 > _this.width) { //check right
              cached.labelX_ = (_this.width - 15 - resolvedX0) / _this.width;
              resolvedX = resolvedX0 + cached.labelX_ * _this.width;
            }
          }
          ...
          if(_labelOffset[1] == 0) {
            if(resolvedY - cached.contentBBox.height <= 0) { // check top 
              ...
              resolvedY = resolvedY0 + cached.labelY_ * _this.height;
            } else if(resolvedY + 10 > _this.height) { //check bottom
              cached.labelY_ = (_this.height - 10 - resolvedY0) / _this.height;
              resolvedY = resolvedY0 + cached.labelY_ * _this.height;
            }
          }
    ...
    function _repositionLabels(d, i, context, _X, _Y, _X0, _Y0, duration, showhide, lineGroup) {
      ...
      if(_X - width <= 0) { //check left
        _X = width;
        cache.labelX_ = (_X - _xScale(cache.labelX0)) / this.width;
      ...
      if(_Y - height * .75 - heightDelta <= 0) { // check top
        ...
        cache.labelY_ = (_Y - _yScale(cache.labelY0)) / this.height;
      } else if(_Y + height * .35 > this.height) { //check bottom
        _Y = this.height - height * .35;
        cache.labelY_ = (_Y - _yScale(cache.labelY0)) / this.height;
      }

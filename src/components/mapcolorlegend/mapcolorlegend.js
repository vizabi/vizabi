...
const MapColorLegend = Component.extend({

  init(config, context) {
    const _this = this;
    this.template = '<div class="vzb-cl-outer"></div>';
    this.name = "mapcolorlegend";
    ...
    this.model_binds = {
      "change:marker.mapcolor.scaleType": function(evt, path) {
        if (!_this._readyOnce || _this.colorModel.isDiscrete()) return;
        _this.updateView();
      },
      "change:marker.mapcolor.palette": function(evt, path) {
  ...
  readyOnce() {
    ...
    this.colorModel = this.model.marker.mapcolor;

...
var Side = Dialog.extend({
  ...
  updateState: function() {    
    ...
    if(this.lastChangedSide && this.state["left"][sideDim] !== this.state["right"][sideDim] && this.state[this.lastChangedSide[0]][sideDim] !== this.lastChangedSide[1]) {
      this.ui.chart.flipSides = !this.ui.chart.flipSides;
    }
  ...
  createList: function(listSel, name, data) {
    ...
    items.append("input")
      ...
      .on("change", function(d, i) {
        ...
        if(!sideEntities.show[sideDim]) {
          if(d[sideDim] !== _this.state[otherSide][sideDim]) {
            sideEntities.showEntity(d);
          }
          sideEntities.showEntity(_this.state[otherSide]);
        } else {
          if(_this.state[name][sideDim] !== _this.state[otherSide][sideDim]) {
            sideEntities.showEntity(_this.state[name]);
          }
          if(d[sideDim] !== _this.state[otherSide][sideDim]) {
            sideEntities.showEntity(d);
          }
        ...
        _this.lastChangedSide = [name, d[sideDim]];
      });

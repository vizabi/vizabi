...
curl -s -H "Content-Type: application/json" -d '{"query":[{"SELECT":["geo","geo.name","time","geo.region", "geo.cat","lex","gdp_per_cap","pop"],"FROM":"humnum","WHERE":{"geo":["*"],"geo.cat":["*"],"time":["1990-2014"]}}],"language":"'$i'"}'  http://waffle.gapminder.org/api/v1/query > waffles/$i/basic-indicators.json
